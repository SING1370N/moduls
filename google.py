...
class GoogleSearchMod(loader.Module):
    ...
    strings = {"name": "Search",
               ...
               "results": "<b>Це отримано з пошуку {}</b> <code>{}</code>:\n\n",
               "result": "<a href='{}'>{}</a>\n\n<code>{}</code>\n"}
    ...
    async def search_mode(self, message, CoreSearch):
        ...
        try:
            ...
            await utils.answer(message, self.strings("results", CoreSearch.__name__, message).format(utils.escape_html(text)) + msg)
        except Exception:
